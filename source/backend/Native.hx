...
class Native
{
	...
	public static function setDarkMode():Void
	{
		...
		untyped __cpp__('
			...
			if (S_OK != DwmSetWindowAttribute(window, attributeDarkModeFallback, &darkMode, sizeof(darkMode))) 
			{
				DwmSetWindowAttribute(window, attributeDarkMode, &darkMode, sizeof(darkMode));
			}
			UpdateWindow(window);
		');
		#end
