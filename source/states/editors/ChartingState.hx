...
{
    private static var lastSongPath:String = null;
    private static var lastSongPosition:Float = 0;
	...
	[
		...
		['Change Character', "Value 1: Character to change (Dad, BF, GF, Ally, Jackal)\nValue 2: New character's name"],
	...
	public static var GRID_PLAYERS = 4;
	...
	var mainBoxPosition:FlxPoint = FlxPoint.get(975, 40);
	...
	override function create()
	{
		...
		{
			//trace(i % GRID_COLUMNS_PER_PLAYER);
		...
		for (i in 0...GRID_PLAYERS)
		{
			...
			icon.scale.set(icon.defaultScale * 0.3, icon.defaultScale * 0.3);
			icon.updateHitbox();
			icon.ID = i + 1;
			add(icon);
			icons.push(icon);
			...
			var text:FlxText = new FlxText((icon.x + icon.width / 2) - FlxG.width / 2, icon.y + icon.height - 90, FlxG.width, '', 12);
			text.setFormat(Paths.font('vcr.ttf'), 12, FlxColor.WHITE, CENTER);
		...
		upperBox = new PsychUIBox(0, 40, 285, 325, ['File', 'Edit', 'View']);
		...
		allyDropDown.list = characterList;
		jackalDropDown.list = characterList;
        ...
        var currentSongPath:String = Song.chartPath != null ? Song.chartPath : PlayState.SONG.song;
        if(currentSongPath == lastSongPath && lastSongPosition > 0)
        {
            Conductor.songPosition = lastSongPosition;
            if(FlxG.sound.music != null) 
            {
                FlxG.sound.music.time = lastSongPosition - Conductor.offset;
	...
	function onChartLoaded()
	{
		...
		allyDropDown.selectedLabel = PlayState.SONG.allyVersion;
		jackalDropDown.selectedLabel = PlayState.SONG.jackalVersion;
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					...
					if(vortexPlaying)
					{
						...
						if(strumNote != null)
						{
							...
							strumNote.specialPlay(Math.max(Conductor.stepCrochet * 1.25, note.sustainLength) / 1000 / playbackRate);
						}
					}
				}
			}
	...
	{
        var newSongPath:String = Song.chartPath != null ? Song.chartPath : song.song;
        if(lastSongPath != null && lastSongPath != newSongPath)
        {
            lastSongPath = null;
            lastSongPosition = 0;
            lastSection = 0;
	...
	function reloadNotes()
	{
		...
		for (secNum => section in PlayState.SONG.notes)
			for (note in section.sectionNotes)
				if(note != null)
				{
					//if (note.length > 5) note.splice(5, 1);
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if(sec != null)
		{
			...
			secondariesTurnCheckBox.checked = sec.secsSection;
	...
	function updateJsonData():Void
	{
		for (i in 1...GRID_PLAYERS+1)
		{
			...
			if (i == 3)
				data = loadCharacterFile(Reflect.field(PlayState.SONG, 'allyVersion'));
			else if (i == 4)
				data = loadCharacterFile(Reflect.field(PlayState.SONG, 'jackalVersion'));
	...
	var _lastSecSection:Null<Bool> = null;	
	function updateHeads(ignoreCheck:Bool = false):Void
	{
		...
		for (i in 0...GRID_PLAYERS)
		{
			...
			//Best check in the humanity existence LMFAO
			charNames[i].text = iconName + '\n'+ (['player', 'opponent', 'ally', 'jackal'])[i];
			if (icon.ID == 3 && (PlayState.SONG.allyVersion == null || PlayState.SONG.allyVersion == '') || icon.ID == 4 && (PlayState.SONG.jackalVersion == null || PlayState.SONG.jackalVersion == ''))
				charNames[i].text += '/gf';
		}
		...
		if(icons.length > 1)
		{
			...
			for (icon in icons)
			{
				/*icon.x = icon.x - icon.width/2 + icon.defaultWidth/2;
				icon.y = icon.y - icon.height/2 + icon.defaultHeight/2;*/
				icon.scale.set(icon.defaultScale * 0.3, icon.defaultScale * 0.3);
				icon.updateHitbox();
				icon.centerOffsets();
			}
			...
			/*if(mustHitSection)
				if (!isSecsSection)
					...
					mustHitIndicator.x = iconP3.x + iconP3.width/2;
			else
				if (!isSecsSection)
	...
	var secondariesTurnCheckBox:PsychUICheckBox;
	...
	function addSectionTab()
	{
		...
		secondariesTurnCheckBox = new PsychUICheckBox(objX + 100, objY, 'Secondaries\nSection', 80, function()
		{
			var sec = getCurChartSection();
		...
		tab_group.add(secondariesTurnCheckBox);

		tab_group.add(new FlxText(beatsPerSecStepper.x, beatsPerSecStepper.y - 35, 100, 'Beats\nPer\nSection:'));
	...
	var allyDropDown:PsychUIDropDownMenu;
	var jackalDropDown:PsychUIDropDownMenu;

	var allowAllyCheckBox:PsychUICheckBox;
	var allowJackalCheckBox:PsychUICheckBox;
	...
	function addSongTab()
	{
		...
		allyDropDown = new PsychUIDropDownMenu(objX + 140, objY + 40, [''], function(id:Int, character:String)
		{
			PlayState.SONG.allyVersion = character;
			updateJsonData();
			updateHeads(true);
			trace('selected $character');
			allowAllyCheckBox.checked = PlayState.SONG.allyVersion != null;
		});

		var prevAllyVersion:String = PlayState.SONG.allyVersion;

		jackalDropDown = new PsychUIDropDownMenu(objX + 140, objY + 80, [''], function(id:Int, character:String)
		{
			PlayState.SONG.jackalVersion = character;
			updateJsonData();
			updateHeads(true);
			trace('selected $character');
			allowJackalCheckBox.checked = PlayState.SONG.jackalVersion != null;
		});

		var prevJackalVersion:String = PlayState.SONG.jackalVersion;

		allowAllyCheckBox = new PsychUICheckBox(objX + 265, objY + 40, '', 80, function()
		{
			if (allowAllyCheckBox.checked) 
			{
				if (prevAllyVersion != null)
					PlayState.SONG.allyVersion = prevAllyVersion;
			} 
			else 
			{
				prevAllyVersion = PlayState.SONG.allyVersion;
				PlayState.SONG.allyVersion = null;
			}
			allyDropDown.text = PlayState.SONG.allyVersion;
			updateJsonData();
			updateHeads(true);
			trace('character: ${PlayState.SONG.allyVersion}');
		});

		allowJackalCheckBox = new PsychUICheckBox(objX + 265, objY + 80, '', 80, function()
		{
			if (allowJackalCheckBox.checked) {
				if (prevJackalVersion != null)
					PlayState.SONG.jackalVersion = prevJackalVersion;
			} 
			else 
			{
				prevJackalVersion = PlayState.SONG.jackalVersion;
				PlayState.SONG.jackalVersion = null;
				//jackalDropDown.text = '';
			}
			jackalDropDown.text = PlayState.SONG.jackalVersion;
			updateJsonData();
			updateHeads(true);
			trace('character: ${PlayState.SONG.jackalVersion}');
		});	

		tab_group.add(allowAllyCheckBox);
		tab_group.add(allowJackalCheckBox);
		...
		tab_group.add(new FlxText(allyDropDown.x, allyDropDown.y - 15, 80, 'Ally:'));
		tab_group.add(new FlxText(jackalDropDown.x, jackalDropDown.y - 15, 80, 'Jackal:'));
		tab_group.add(jackalDropDown);
		tab_group.add(allyDropDown);
	...
	function addEditTab()
	{
		...
		var btnY = 1;
		var btnWid = Std.int(tab.width);
		var btnHei = Std.int(tab.height);

		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Undo', undo, btnWid, btnHei);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY += btnHei;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Redo', redo, btnWid, btnHei);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		if(SHOW_EVENT_COLUMN)
		{
			btnY++;
			btnY += btnHei;
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Lock Events', btnWid, btnHei);
		...
		btnY++;
		btnY += btnHei;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Autosave\n  Settings...', btnWid, btnHei + 10);
		btn.onClick = function()
		{
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
	...
	/*var totalGridWidth:Int = Std.int(GRID_SIZE * GRID_COLUMNS_PER_PLAYER * GRID_PLAYERS);
	var waveformWidth:Int = Std.int(totalGridWidth / 2); 
