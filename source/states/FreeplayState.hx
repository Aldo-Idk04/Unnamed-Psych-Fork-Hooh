...
class FreeplayState extends MusicBeatState
{
	...
	private static var prevBpm:Float = Conductor.bpm;
	...
	override function create()
	{
		...
		//Just to reset the new bpms
		Conductor.bpm = MainMenuState.mainBPM;
	...
	override function update(elapsed:Float)
	{
		...
		else if(FlxG.keys.justPressed.SPACE)
		{
			if(instPlaying != curSelected && !player.playingMusic)
			{
				...
				#else
					var path:String = Paths.formatToSongPath(PlayState.SONG.song);
				#end
		...
		else if (controls.ACCEPT && !player.playingMusic)
		{
			...
			var realSongToLoad:String = Highscore.formatSong(songLowercase, curDifficulty);
		...
		//Logic to reset the anims
		for (i in -2...3) 
		{
    	var index = FlxMath.wrap(curSelected + i, 0, iconArray.length - 1);
    	var icon = iconArray[index];
			if (icon != null && icon.graphic.key != null && icon.active) 
			{
				var shouldBe = (i == 0 && icon.hasAnimation('winning')) ? 'winning' : 'idle';
				var prevAnim = icon.animation?.curAnim?.name ?? "";
				if (icon.animPerBeat > 0 && prevAnim != shouldBe)
				{
					icon.playAnim(shouldBe, true);
				}
				else if (icon.animPerBeat <= 0)
				{
					icon.playAnim(shouldBe);
				}
			}
		}
	}
	...
	override function beatHit() 
	{
		...
		if (bg != null && ClientPrefs.data.camZooms)
		{
			bg.scale.set(bg.scale.x + 0.02, bg.scale.y + 0.02);
			FlxTween.tween(bg.scale, {x: 1, y: 1}, Conductor.crochet * 0.001 * 0.48, {ease: FlxEase.quintOut});
		}
		for (i in -2...3) 
		{
			var index = FlxMath.wrap(curSelected + i, 0, iconArray.length - 1);
			...
			if (icon != null && icon.graphic.key != null && icon.active) 
			{
				...
				{
					//icon.alpha = 1;
					icon.scale.set(icon.defaultScale + 0.15, icon.defaultScale + 0.15);
					//FlxTween.tween(icon.scale, {x: icon.defaultScale, y: icon.defaultScale}, FlxMath.bound(0.42 * 60 * FlxG.elapsed, 0, 1), {ease: FlxEase.quintOut});
					FlxTween.tween(icon.scale, {x: icon.defaultScale, y: icon.defaultScale}, Conductor.crochet * 0.001 * 0.48, {ease: FlxEase.quintOut});
					if (icon.animPerBeat > 0 && curBeat % icon.animPerBeat == 0 && icon.animation.name != icon.getCharacter()) 
					{
						icon.playAnim(icon.hasAnimation('winning') ? 'winning' : 'idle', true);
					}
				} 
				else 
				{
					//icon.alpha = 0.6;
					if (icon.scale.x != icon.defaultScale)
						icon.scale.set(icon.defaultScale, icon.defaultScale);
					if (icon.animPerBeat > 0 && curBeat % icon.animPerBeat == 0 && icon.animation.name != icon.getCharacter()) 
					{
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		for (num => item in grpSongs.members)
		{
			...
			if (icon.graphic.key != null && icon.active)
			{
				if (icon.animation.name == icon.getCharacter() && icon.animation.curAnim.frames.length == 3)
					icon.animation.curAnim.curFrame = num == curSelected ? 2 : 0;
			}
		}
